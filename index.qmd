...
author: "Carole Metekoua" # Remember to add your name!
theme: united
---
...
plot2 <- cantril %>% 
  ...
  mutate(rank= ifelse(rank<=10 , 'Top 10', 'Bottom 10'),
         rank = factor(rank, levels=c('Top 10', 'Bottom 10'), ordered = TRUE),
         Country = factor(Country, levels = rev(unique(Country)))) %>%
  ggplot(aes(x=Country, y=`Cantril ladder score`, fill = rank)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous(limits = c(0,10),
                     breaks = seq(0,10,2)) +
